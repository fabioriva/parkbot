...
export default function Screen(props) {
  ...
  const { comm } = useComm(`${process.env.NEXT_PUBLIC_WEBSOCK_URL}/${props.aps}/info`)

  const [screen, setScreen] = React.useState(props.json);
  const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/${props.aps}/dss/${id}`
    ...
    <React.Fragment>
      <Container maxWidth={false} className="screen">
        <Box className="screen-header">
          <Grid container>
            <Grid item xs={4}>
              <Item sx={{ backgroundColor: "#adb5bd", color: "#000" }}>SOTEFIN SA</Item>
            </Grid>
            <Grid item xs={4}>
              <Link href={`/${props.aps}/dss`}>
                <a style={{ textDecoration: 'none', color: 'white' }}><Item sx={{ backgroundColor: "#0d6efd" }}>{screen.name}</Item></a>
              </Link>
            </Grid>
            <Grid item xs={4}>
              {comm ? (
