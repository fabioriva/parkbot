...
export const getServerSideProps = async ({ locales, req, res }) => {
  const ns = ['map']
  const { locale, user } = await getUser(req, res)
  if (!user) {
    return {
      redirect: {
        destination: '/signin',
  ...
  const json = await fetcher(`${BACKEND_URL}/map`)
  return {
    props: {
