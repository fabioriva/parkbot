...
import Overview from 'src/components/overview/ironbank'
...
export async function getServerSideProps (ctx) {
  ...
  const props = await authSSR(ctx, 'ironbank', OVERVIEW)
  if (props.notFound || props.redirect) return props

  const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/ironbank/overview`
